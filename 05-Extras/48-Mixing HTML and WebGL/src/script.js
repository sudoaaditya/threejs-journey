...
const loadBarItem = document.querySelector('.loading-bar');
const loadingManager = new THREE.LoadingManager(
    () => {
        window.setTimeout( () => {
            gsap.to(overlayMat.uniforms.uAlpha, { duration: 3, value: 0});
            loadBarItem.classList.add('ended');
            loadBarItem.style.transform = '';
        }, 500);

        window.setTimeout(() => sceneReady = true, 3000);
    },
    (url, itemsLoaded, totalItems) => {
        const progRatio = itemsLoaded/totalItems;
        loadBarItem.style.transform = `scaleX(${progRatio})`;
    }
);
...
const points = [
    ...
    {
        position: new THREE.Vector3(0.5, 0.0, -1.6),
