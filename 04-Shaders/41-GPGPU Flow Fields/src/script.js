...
gpgpu.comutation = new GPUComputationRenderer(gpgpu.size, gpgpu.size, renderer);

// Base particles
const baseParticleTexture = gpgpu.comutation.createTexture();
...
//particles variables
gpgpu.particlesVariable = gpgpu.comutation.addVariable("uParticles", gpgpuParticlesShader, baseParticleTexture);
gpgpu.comutation.setVariableDependencies(gpgpu.particlesVariable, [gpgpu.particlesVariable]);
...
gpgpu.comutation.init();


//Debug GPGPU
gpgpu.debug = new THREE.Mesh(
    new THREE.PlaneGeometry(3, 3),
    new THREE.MeshBasicMaterial({ map: gpgpu.comutation.getCurrentRenderTarget(gpgpu.particlesVariable).texture })
);
...
particles.geometry.setAttribute(
    "aSize",
    new THREE.Float32BufferAttribute(particlesSize, 1)
);
...
const tick = () => {
    ...
    gpgpu.comutation.compute();
    particles.material.uniforms.uParticlesTexture.value =
        gpgpu.comutation.getCurrentRenderTarget(gpgpu.particlesVariable).texture;
