...
const debugObj = {
    ...
    createBox: () => createBox(Math.random(), Math.random(), Math.random(), {x: (Math.random() - 0.5) * 3, y: 3, z: (Math.random() - 0.5) * 3}),
    reset: () => resetScene()
}
...
const playHitSound = (collision) => {
    ...
    const imapctStregth = collision.contact.getImpactVelocityAlongNormal();

    if(imapctStregth > 1.5) {
...
// simpler way to add defaultContactMaterial in world
...
const floorBody = new CANNON.Body({
    mass: 0, // 0 implies this body is static it will not move,
    material: defaultMaterial
});
...
const boxMatGeo = new THREE.MeshStandardMaterial({
    metalness: 0.3,
    roughness: 0.4,
    envMap: environmentMapTexture,
    envMapIntensity: 0.5
...
const createBox = (w, h, d, position) => {
    // THREE MESH
    const mesh = new THREE.Mesh(boxGeo, boxMatGeo);
